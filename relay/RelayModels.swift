...
struct RelayFilter {
    ...
    func matches(_ event: RelayEvent) -> Bool {
        if let ids, !ids.isEmpty, !ids.contains(where: { event.id.hasPrefix($0) }) {
            return false
        }

        if let authors, !authors.isEmpty, !authors.contains(where: { event.pubkey.hasPrefix($0) }) {
            return false
        }

        if let kinds, !kinds.isEmpty, !kinds.contains(event.kind) {
            return false
        }

        if let since, event.created_at < since {
            return false
        }

        if let until, event.created_at > until {
            return false
        }
